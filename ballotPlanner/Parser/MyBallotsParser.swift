...
class MyBallotsParser {
  ...
  func getAllMyBallots(completion: @escaping([Candidate]) -> Void) {
  ...
  func getBallotCandidates(completion: @escaping([Candidate]) -> Void) {
    let db = Firestore.firestore()
    self.group4.enter()
...
//          let newBallotCandidate = Candidate(first_name: document.get("first_name") as! String, last_name: document.get("last_name") as! String)
          let newBallotCandidate = Candidate(first_name: document.get("first_name") as! String, last_name: document.get("last_name") as! String, party_affiliation: document.get("party_affiliation") as! String, image: nil, contact_twitter: document.get("contact_twitter") as! String, summary: document.get("summary") as! String)
          self.myBallots.append(newBallotCandidate)
        }
        self.group4.leave()
      }
      completion(self.myBallots)
