...
class ElectionDatesViewModelTests: XCTestCase {
  ...
  func testNumberofRows(){
    ...
    viewModel.electionDates = dates
    
    XCTAssertEqual(viewModel.numberOfRows(), 3)
  }
  
  func testTitleForRowAtIndexPath(){
    let date1 = ElectionDate(date: "123", description: "789", name: "456")
    let date2 = ElectionDate(date: "ABC", description: "DEF", name: "GHI")
  ...
  func testsDetailViewModelForRowAtIndexPath(){
    let date1 = ElectionDate(date: "123", description: "789", name: "456")
    let date2 = ElectionDate(date: "ABC", description: "DEF", name: "GHI")
    let dates = [date1, date2]
    ...
    XCTAssertEqual(detailVM.electionDate.name, date2.name)
//    ElectionDateDetailViewModel(electionDate: electionDates[indexPath.row])
  }
  func testMERefresh(){
